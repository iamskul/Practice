...
while True :
    ...
    outfileLiveData.write("\n")
    if abs(int(sum(data_streamPriceW1)/window1) - int(sum(data_streamPriceW3)/window3)) < 3 and crossoverFlag == 0 :
        crossoverFlag = 1
        DECISION.write(" Crossover has happened at - ")
        DECISION.write(productPrice)
        DECISION.write("\n")
    elif crossoverFlag == 1 and int(sum(data_streamPriceW3)/window3) - int(sum(data_streamPriceW1)/window1) >= 3 and buyOrderPlaced == 0 and int(sum(data_streamPriceW2)/window2) - int(sum(data_streamPriceW3)/window3) >= 1.25 :
        DECISION.write("Generating a BUY Quote at - ")
        DECISION.write(productPrice)
        DECISION.write("\n")
        buyOrderPlaced = 1
    elif buyOrderPlaced == 1 and int(sum(data_streamPriceW1)/window1) - int(sum(data_streamPriceW3)/window3) >= 3 and int(sum(data_streamPriceW3)/window3) - int(sum(data_streamPriceW2)/window2) >= 1.25:
        DECISION.write("Generating SELL Quote at - ")
        DECISION.write(productPrice)
        DECISION.write("\n")
        buyOrderPlaced = 0
        crossoverFlag = 0

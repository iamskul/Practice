...
while True :
    ...
    outfileLiveData.write("\n")
    if int(sum(data_streamPriceW1)/window1) == int(sum(data_streamPriceW3)/window3) and crossoverCount < 7 :
        crossoverCount += 1
    elif int(sum(data_streamPriceW1)/window1) == int(sum(data_streamPriceW3)/window3) and crossoverCount == 7 and buyOrderVal == 0 :
        buyOrderVal = float(productPrice)
        DECISION.write(" Begining Crossover and Buy Quote at - ")
        DECISION.write(productPrice)
        DECISION.write("\n")
        crossoverCount = 0
    elif int(sum(data_streamPriceW1)/window1) == int(sum(data_streamPriceW3)/window3) and crossoverCount == 7 and buyOrderVal != 0 and int(buyOrderVal) < int(sum(data_streamPriceW1)/window1) :
        if float(productPrice) >= sellMaxVal :
            DECISION.write(" Crossover Sell all at - ")
            DECISION.write(productPrice)
            DECISION.write("\n")
            sellMaxVal = 0
        ...
        buyOrderVal = float(productPrice)
        crossoverCount = 0
    elif buyOrderVal != 0 and ((int(sum(data_streamPriceW1)/window1) - int(sum(data_streamPriceW3)/window3) >= 2 and int(sum(data_streamPriceW3)/window3) - int(sum(data_streamPriceW2)/window2) >= 4) or buyOrderVal - float(productPrice) >= 13) :
        DECISION.write(" Saving SELLABLE max limit Quote at - ")
        if sellMaxVal == 0 :
            sellMaxVal = buyOrderVal
        elif sellMaxVal < buyOrderVal :
            sellMaxVal = buyOrderVal
        DECISION.write(str(sellMaxVal))
        DECISION.write("\n")
        buyOrderVal = 0
        crossoverCount = 0
